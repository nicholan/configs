...
async function chExtension(folderPath, targetExt, outputExt, recursive = false) {
	try {
		...
		for (const file of files) {
			...
			if (isDir) {
				// If recursive is false => continue checking remaining files.
				if (!recursive) continue;

				// If recursive is true => modify files within nested folders.
